...
namespace lab8
{
    class TcpServer
    {
            ...
            using (var networkStream = tcpClient.GetStream())
            {
                ...
                switch (splittedRequest[0].ToLower())
                {
                    ...
                    case "logout":
                        ...
                        break;
                    case "register":
                        await RegisterProtocol(networkStream, splittedRequest);
                        break;
                    default:
        ...
        /// <summary>
        /// Obsługa protokołu rejestracji
        /// </summary>
        public async Task RegisterProtocol(NetworkStream networkStream, string[] splittedRequest)
        {
            string login = splittedRequest[1].ToLower();
            if (!users.ContainsKey(login))
            {
                string password = splittedRequest[2];
                users.Add(login, password);
                await EncodeAndWriteServerResponse(networkStream, "New account created");
            }
            else
            {
                await EncodeAndWriteServerResponse(networkStream, "user already exist !");
            }
        ...
        private static string GenerateIdKey()
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                serwerResponseString += (char)random.Next(48, 122);
            }
